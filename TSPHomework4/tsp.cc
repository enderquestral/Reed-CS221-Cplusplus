...
//////////////////////////////////////////////////////////////////////////////
// Threaded GA search, uses multiple threads to do a GA search.
Cities::permutation_t
          ...
          unsigned nthread = 1)
{
  ...
  auto best_ordering = Cities::permutation_t(cities.size());
  auto best_mutex = std::mutex();
  auto old_best_dist = best_dist; //because is_improved changes best_dist
  ...
  auto run_one_thread = [&]() {
    ...
    for (long i = 1; i <= (iters/pop_size)/nthread; ++i) {
      deme.compute_next_generation();    // generate next generation
      old_best_dist = best_dist;

      // Find best individual in this population
      const auto ordering = deme.get_best()->get_ordering();
      if (is_improved(cities, ordering, old_best_dist, i * pop_size)) {
        auto guard = std::lock_guard(best_mutex);
        //std::cout <<"test" <<"\n";
        if (is_improved(cities, ordering, best_dist, i * pop_size)) //side effects?
        {
          //std::cout <<"test" <<"\n";
          best_ordering = ordering;
          
          //std::cout << "The best ordering at 2 is: "<< best_ordering[2] << "\n";
        }
      }
    }
  };
  ...
  //std::cout << "The best ordering is: " << best_ordering << "\n";
  return best_ordering;
