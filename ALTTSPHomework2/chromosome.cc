...
void Chromosome::mutate() {
  // Add your implementation here
    ...
    //std::cout<< randomplace1 <<"\n";
    //std::cout<< randomplace2 <<"\n";
                                   ...
                                   unsigned b, unsigned e) const
{
  ...
  for ( ; i < p1->order_.size() && j < p2->order_.size(); ++i) {
    if (i >= b and i < e) {
      ...
      j++;
      //std::cout << "2...: " << child->order_[i] << "\n";
    }
  }
  ///ADDED
  //for (auto i: child->order_){
    //std::cout << "Printing child order...: " << i << "\n";
  //}
  ///ADDED
...
double Chromosome::get_fitness() const {
    ...
    */
    ...
    //1 OVER TOTAL DISTANCE +1 ? 
    //MAXIMUM DOUBLE / NUMBER OF CITIES, then subtract distances?
    auto largestNum = 100000 / cities_ptr_->size();
...
bool Chromosome::is_valid() const {
    ...
    /*for (unsigned i = 0; i < placeholder.size(); ++i)
    {
        if(std::find(placeholder.begin()+i, placeholder.end(), i) != placeholder.end()){
            //element found again in placeholder permutation
            return false;
        }
        if(i > order_.size()){ //no indicies above the ranve of the chromosome.
            return false;
        }
    }*/

    /*for(auto i : order_){ //order_ is a vector of unsigned ints.
        if(std::find(placeholder.begin(), placeholder.end(), i) != placeholder.end()){
            //element found again in placeholder permutation
            return false;
        }
        if(i > order_.size()){ //no indicies above the ranve of the chromosome.
